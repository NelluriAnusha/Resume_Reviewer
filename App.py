...
def insert_data(name,email,res_score,timestamp,no_of_pages,reco_field,candidate_level,skills,recommended_skills,courses):
    DB_table_name = 'candidate_data'
    insert_sql = "insert into " + DB_table_name + """
    values (0,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
    rec_values = (name, email, str(res_score), timestamp,str(no_of_pages), reco_field, candidate_level, skills,recommended_skills,courses)
...
def run():
    ...
    selected_users = ["Candidate", "Employer"]
    choice = st.sidebar.selectbox("Select the option in below menu:", selected_users)
    logo_img = Image.open('./Logo/resume_logo.jpg')
    ...
    # Creating the table
    database_table_name = 'candidate_data'
    create_table_sql = "CREATE TABLE IF NOT EXISTS " + database_table_name + """
    ...
    if choice == 'Candidate':
        pdfFile = st.file_uploader("Upload your CV to get personalized job recommendations", type=["pdf"])
        if pdfFile is not None:
            save_img_loc = './Uploaded_Resumes/'+pdfFile.name
            with open(save_img_loc, "wb") as f:
                f.write(pdfFile.getbuffer())
            show_pdf(save_img_loc)
            resume_Extracted_data = ResumeParser(save_img_loc).get_extracted_data()
            if resume_Extracted_data:
                ## Getting the whole resume data
                resume_text = pdf_reader(save_img_loc)

                st.header("**** Resume Analysis ****")
                st.success("Hello "+ resume_Extracted_data['name'])
                st.subheader("**Your Basic Details**")
                try:
                    st.text('Name: '+resume_Extracted_data['name'])
                    st.text('Email: ' + resume_Extracted_data['email'])
                    st.text('Contact: ' + resume_Extracted_data['mobile_number'])
                    st.text('Resume pages: '+str(resume_Extracted_data['no_of_pages']))
                except:
                    pass
                candidate_level = ''
                if resume_Extracted_data['no_of_pages'] == 1:
                    candidate_level = "Fresher"
                    st.markdown( '''<h4 style='text-align: left; color: #d73b5c;'>You are looking Fresher..</h4>''',unsafe_allow_html=True)
                elif resume_Extracted_data['no_of_pages'] == 2:
                    candidate_level = "Intermediate"
                    st.markdown('''<h4 style='text-align: left; color: #1ed760;'>Great!! You are at intermediate level..</h4>''',unsafe_allow_html=True)
                elif resume_Extracted_data['no_of_pages'] >=3:
                    candidate_level = "Experienced"
                    st.markdown('''<h4 style='text-align: left; color: #fba171;'>Awesome!! You are at experience level..''',unsafe_allow_html=True)

                st.subheader("**Skills Recommendation💡**")
                ## Skill shows
                keywords = st_tags(label='### Skills that you have',
                text='See our skills recommendation',
                    value=resume_Extracted_data['skills'],key = '1')
                ...
                for i in resume_Extracted_data['skills']:
                ...
                insert_data(resume_Extracted_data['name'], resume_Extracted_data['email'], str(resume_score), timestamp,
                              str(resume_Extracted_data['no_of_pages']), reco_field, candidate_level, str(resume_Extracted_data['skills']),
